...
export class DNAConfig {
    ...
    public async deploy(console?: Console) {
        const sourceDir = path.resolve(this.sourceDir || ".");
        ...
        let dockerFilePath = `${sourceDir}/${this.dockerfile || "Dockerfile"}`;
        ...
        try {
            this.preStructreAndValidateDockerFile(dockerFilePath, sourceDir, this.allowRunCommands || false);
        }
        catch(e) {
        ...
        // Create keys:.
        const {publicKey, secretKey} = nacl.box.keyPair.fromSecretKey(seedkey);
        ...
        const packageName = this.getPackageName(sourceDir);

    }

    preStructreAndValidateDockerFile(dockerFilePath: string, sourceDir: string, allowRunCommands: boolean) {
